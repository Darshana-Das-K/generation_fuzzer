...
import struct
import fcntl
from interesting import detect_packet_deviation
...
def generate_pcap_authentication_request(authentication_request_generated_file, session_id):
   ...
   # pap_file = "/Users/darshanadask/main_project/19_Nov/authenticate_request0.areq"
    ...
    eth_frame = Ether(src="00:00:24:cd:1a:c0", dst="40:7b:1b:00:12:10", type=0x8864)  # 0x8864 = PPPoE Session
...
def generate_discovery_phase_packet(generated_discovery_phase_packet, dst_address):
    #padi_file = "testcases/padi_packet.padi"
    with open(generated_discovery_phase_packet, "rb") as f:
        padi_data = f.read()

    # Construct Ethernet frame
    eth_frame = Ether(src="00:00:24:cd:1a:c0", dst=dst_address, type=0x8863)  # 0x8863 = PPPoE Discovery
...
def fuzzer_bot(interface, padi_generated_file, padr_generated_file, configure_request_generated_file, echo_request_generated_file, authentication_request_generated_file, padt_generated_file):
    ...
    padi_packet= generate_discovery_phase_packet(padi_generated_file, "ff:ff:ff:ff:ff:ff")
    try:
    ...
    padr_packet = generate_discovery_phase_packet(padr_generated_file, "40:7b:1b:00:12:10")
    try:
   ...
   # sock = socket.socket(socket.AF_PACKET, socket.SOCK_RAW, socket.htons(ETH_P_PPP_SESS))
    ...
    padt_packet= generate_discovery_phase_packet(padt_generated_file_with_session_id, "40:7b:1b:00:12:10")
    try:
